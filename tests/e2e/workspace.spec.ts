...
const tools = [
  { path: "/workspace/tools/meta-audit", selector: { label: "Ad Account ID", value: "123456" }, submit: /Run Audit/i, verify: /Meta account audit completed/i },
...
test("workspace tools submit successfully", async ({ page }) => {
  for (const tool of tools) {
    ...
    } else if (tool.selector.placeholder) {
    ...
    await expect(page.getByText(tool.verify)).toBeVisible({ timeout: 5000 });
  }
});
